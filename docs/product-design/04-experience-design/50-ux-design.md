...
| **P4** | **Every non-Healthy state is honest, distinct, and named.** No blended "needs attention." Each holding state gets its own shape/badge **and** a plain-language sentence that names the failing host ("Codex needs sign-in; Claude is fine"). State is never conveyed by color alone. | "Roll everything up into one severity indicator to reduce clutter." | US-B08, US-B09; E21; MTM-2; a11y bar below |
...
│       ├─ Status sentence          (top line — plain language, names the failing host)
│       ├─ Primary action           (contextual: Sync now / Repair / Sign in / Finish setup)
│       ├─ Secondary actions        (What changed · Add a skill · Open cheat-sheet)
│       ├─ Hosts ▸                   (per-host submenu — Claude / Codex / Shared)
...
**Navigation model.** There is **no navigation** for Bob — the dropdown is a flat, single-level menu
(Hosts ▸ is the only submenu, kept ≤5 rows per Miller's Law). The wizard is linear and mostly
auto-advancing. Only the Admin window has tabbed sections, because Raj is technical and does real work
there. This asymmetry is the Bob-first decision made structural.

**What the user sees first / what waits.** First glance = the glyph shape + (if not Healthy) one
sentence. Everything else — what changed, per-host detail, preferences — waits behind a click. This is
progressive disclosure taken to its limit: the periphery (the glyph) carries the whole steady-state
signal; the foreground (the dropdown) opens only when Bob decides to look.
...
3. If Healthy, he looks away — done. If badged, one click reveals a single plain sentence naming the
   host and the one action (Flow 6).
...
| 2 | **Signed-out** | *Auth-needed* | `auth[].state ∈ {expired,revoked}` for a required layer | solid **+ key** | "Codex needs sign-in; Claude is fine." | **Sign in…** (own data) | Bob (own data) | US-B08/B09, E22 |
| 3 | **Needs-attention** | *Needs-you* | `doctor.status == needs-attention` (warn / unhealable fail survived auto-repair) | solid **+ amber triangle** | Names the finding + failing host in one sentence | **Repair…** (CLI verb) | only if Bob-actionable | US-B09, US-B10 |
...
sentence** that names the failing host.* (P2 + P4; US-B07, US-B09, US-O05, US-O06.)

**Worst-wins, per-host attribution.** The single glyph shows the *worst* of the two hosts (precedence
column order), but the sentence and the **Hosts ▸** submenu attribute *per host* — Bob learns a fact
("Codex needs sign-in; Claude is fine"), never a blur (US-B08, E21). Updates are per-host transactional,
so a Claude-success / Codex-fail leaves a consistent lock per host.

### Microinteractions (Trigger → Rules → Feedback → Loops)

| Interaction | Trigger | Rules | Feedback | Loops / over time |
|---|---|---|---|---|
| **Status transition** | fresh CLI JSON changes the parsed state | transition **only** from fresh JSON (never inferred); worst-wins | glyph cross-fades shape/badge (~180ms); VoiceOver announces the new sentence via live region | Bob learns the shapes; over weeks the badge alone is enough — text becomes confirmation, not discovery |
| **Syncing ring** | an `update`/`repair` spawns | one invocation in flight per host; coalesce triggers | thin ring rotates on the glyph; top line streams the phase name | most syncs are silent+fast; the ring is a reassurance, not an alarm |
...
|---|---|---|
| **Popover sizing** | Fixed comfortable width (~320pt); height grows to content, caps with internal scroll only for the Needs-attention finding list. Anchored to the glyph, arrow pointing up. | Miller's Law — the menu stays ≤7 top-level rows; only a finding list may exceed and it scrolls |
...
2. **VoiceOver labels on the glyph and every dropdown item.** The tray item exposes an
   `NSAccessibility` label that reads the *current status sentence* ("Codex needs sign-in; Claude is
   fine"), not "app icon". Every menu row has a label naming its action and the CLI effect in plain
   language ("Repair — fixes the finding IT can see"). Badges are described, never left as decoration.
...
4. **Full keyboard operability — dropdown and wizard.** The dropdown is reachable and fully operable by
   keyboard (open, arrow through rows, Enter to invoke, Esc to close); documented tab order in the
