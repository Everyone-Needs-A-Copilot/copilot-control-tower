...
| 4 | **Security inherited, never weakened** | No `--skip-verify`, no `--force`. Security config honored **only** from the forced/managed MDM domain. Trust roots are compiled-in code, not config. | `managed/forced.rs`, `managed/keys.rs`, `updater/trust.rs`, FF-M5-* |
...
| 6 | **One-way inheritance; secrets never travel** | Secrets never enter inheritance content or any git repo (only `requires_secret: <NAME>` refs). No cross-tier write path. Sync is pull-only/downward. Fail-closed leak-scan on every writable push. | `settings/guard.rs`, `settings/authoring.rs`, `telemetry/schema.rs`, FF-M7-CONTENTFREE |
...
- **Parse-never-compute:** Rust core spawns `cc <verb> --json` by an **absolute,
  ...
  compute the merge.
- **4 products × 4 layers, product-first:** Knowledge / CLI / Claude / Codex ×
  foundation / org / dept / personal. The UI is product-first (product dropdown,
  then layer detail).
...
- **MDM forced-domain** — generator unit-tested; not verified against a live enrolled Mac.
