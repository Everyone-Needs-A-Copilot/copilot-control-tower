...
- **Four products** (Knowledge Copilot, CLI Copilot, Claude Copilot, Codex Copilot — config-driven, not a hardcoded count) are **bundles of dimensions**: Claude/Codex Copilot own agents/skills/commands/protocol/memory/tasks; Knowledge Copilot owns knowledge; CLI Copilot owns integrations. The resolver folds *inside each product stack, then per dimension*, so resolution identity is `(product, dimension, item)`. Adding a product is additive by construction, and a same-named Claude and Codex item can coexist without either entering the other's shadow chain (invariant: a layer belongs to exactly one product × tier).
...
There is **one** resolution engine (from `02-four-tier-and-github-topology.md` §4 *(external — source-repo appendix; see [ecosystem-links.md](./ecosystem-links.md))*): load the manifest, group layers by `product`, sort each product stack by `rank`, and per **dimension** apply that dimension's semantics. It then **materializes** the winning set into allowlisted discovery paths for that product (copy, not read-time merge — the host is layer-unaware). Rank uniqueness and ordering are enforced inside each product stack: Claude rank 10 and Codex rank 10 are valid together, while two Claude rank-10 layers are not. Every layer declares a `product` field and every resolved item carries it, so `resolve`/`doctor` output is directly groupable without reconstructing product identity from repo names.
