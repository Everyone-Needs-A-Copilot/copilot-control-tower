...
- **Conflict chooser (when two of the user's Macs diverged a key):** a small inset card or
  ...
  named by machine and by when the value was set** (for example "Pablo's Mac Studio, set
...
```
  ...
  |  [laptopcomputer]  This Mac (Pablo's MacBook Pro)         |   Enrolled  (no Remove on This Mac)
  |  [desktopcomputer] Pablo's Mac Studio        Enrolled  (-) |   Remove = minus.circle
  |  [laptopcomputer]  Pablo's old MacBook Air   Not syncing(+)|   Enroll = plus.circle
  +-----------------------------------------------------------+
  WHAT SYNCS
...
when each set the value ("Pablo's Mac Studio, set yesterday" versus "This Mac, set today") and asks
