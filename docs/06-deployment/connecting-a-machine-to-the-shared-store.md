...
- **The non-technical end-user experience is a separate, still-open design question.** `docs/03-design/connect-experience-walkthrough.md` (WP-396) and its rendered walkthrough (WP-398) lay out two candidate futures — a "membership is access" model where a person never sees a credential moment at all, versus a "claim code" approval flow — and neither is built. This runbook describes the mechanism both futures would eventually sit on top of, not either finished experience.
