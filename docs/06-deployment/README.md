...
| `.mobileconfig` generator, managed login-item force-approve, forced-domain reader, forced-key deprovision trigger | **Superseded by D4 - MDM is dropped, including the seam.** That code lived in `src-tauri/` (`src/mobileconfig/`, `src/loginitem/`, `src/managed/forced.rs`, `src/routing/deprovision_trigger.rs`), which was removed from this repository in commit `chore: remove retired src-tauri tree`; it survives only in git history. It is **not** part of the target deployment model this directory documents; do not build a new standup around it. | see the individual `m5-owner-gated-batch.md` entries below for what of M5 survives D4 and what doesn't |
