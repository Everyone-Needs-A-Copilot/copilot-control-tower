...
Windows re-skin instead of security. Superseded in scope (not content) by
...
> part of the ADR-M9-004 row (a mitigation that assumed a central MDM push)
> describe Windows-side mirrors of the macOS MDM design this repo has since
> dropped. They are flagged here as pending reconciliation with D4, not
> rewritten to a new design — that is a Windows-parity-owner decision, not a
> docs fix, and is separate from the native-macOS-direction note in
> `windows-bringup.md`.
...
| Area | Buildable now (macOS-green, `cfg`-gated) | Owner-gated (needs Windows box / EV cert) |
|---|---|---|
...
| Forced config (ADR-M9-003, parity Q2), **pending reconciliation with D4** (see banner above; this row's premise, a forced/managed config domain to mirror, no longer matches the deployment model) | `HKLM\Software\Policies\ENAC\ControlTower` reader, gated on an enrollment-detection function, same `managed::keys::MANAGED_KEYS` registry, same fail-closed default when not enrolled; **@agent-sec review of this ADR is a prerequisite to Accepted status, itself not owner-gated** (it's a code review, not a hardware need) | Real domain-enrollment detection across every state a real deployment can produce; confirming a local admin on an *unenrolled* box truly cannot spoof "enrolled" |
...
| Code signing (ADR-M9-004, parity Q3) | `signtool verify` at the same pinned-SHA call site `codesign`/`spctl` occupies today; updater pre-promote check rewritten as Authenticode-valid + version-monotonicity; signing scripts authored (not run) | EV cert acquisition + real `signtool sign` execution; observing actual SmartScreen behavior for a freshly-signed binary. **Pending reconciliation with D4:** the air-gapped-fleet mitigation this ADR names (a centrally-pushed SmartScreen allow-list) assumed a device-management push that no longer exists in this repo's deployment model; whether an equivalent exists under the repo-access model is unresolved, not just unverified |
...
| Notifications (§3 Q4) | Toast notification wiring via Tauri's plugin; the conservative default (fallback-to-IT-channel treated as primary on Windows) implemented as the actual code path, not just documented | Real test of whether per-app notification permission can be centrally forced on Windows 11. **Pending reconciliation with D4:** the console this was meant to test through no longer applies here |
