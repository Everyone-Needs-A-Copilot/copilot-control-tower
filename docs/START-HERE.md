...
- **The CLI engine (`cc`) is a separate repo, actively developed.** It lives at `/Volumes/Dev/Sites/CSE/claude-copilot`, under `tools/cc`, currently on branch `feat/adopt-and-project-setup`. The app bundles a signed copy of it at `Contents/Resources/cc` inside the app bundle — the shipped 0.6.9 release carries helper version **cc 2.12.7**.
