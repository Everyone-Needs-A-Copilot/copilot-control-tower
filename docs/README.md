...
| [`01-architecture/`](01-architecture/) | The hardened `architecture.md` + the `cli-contract.md` prerequisite; `inheritance-and-publish.md` (writable tiers + `copilot publish`); `error-taxonomy.md`; versioned [`schemas/`](01-architecture/schemas/) (the `--json` contract source of truth) | Seeded |
...
| [`05-security/`](05-security/) | `credentials-and-boundary.md` (secret carrier, leakage wall, push creds) + `threat-model.md` (app-level STRIDE+DREAD); `security-and-trust.md` indexes them | Seeded (credentials + threat model done) |
| [`06-deployment/`](06-deployment/) | Per-MDM deployment guides, managed-config reference, offline path, deprovision runbook | Stub |
| [`07-contributing/`](07-contributing/) | Dev setup, build, signing, self-update, release process | Stub |
