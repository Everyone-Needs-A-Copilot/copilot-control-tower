...
| [`05-security/`](05-security/) | `credentials-and-boundary.md` (secret carrier, leakage wall, push creds) + `self-service-store-provisioning.md` (GitHub-verified store access, DESIGN FOR REVIEW) + `threat-model.md` (app-level STRIDE+DREAD) + `incident-response.md` (maintainer runbook); `security-and-trust.md` indexes them. Root `SECURITY.md` points here | Seeded |
