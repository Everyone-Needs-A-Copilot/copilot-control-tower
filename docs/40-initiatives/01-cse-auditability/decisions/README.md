...
| DEC-4 | [`DEC-4-stale-clones.md`](DEC-4-stale-clones.md) | TASK-118 (R-9) — **RULED 2026-07-14: remove the `frosty-perlman` worktree — EXECUTED, `completed`** | Delete or keep the stale `shared-docs`/`conversations-copilot` clone artifacts? |
| DEC-5 | [`DEC-5-configure-or-cut-services.md`](DEC-5-configure-or-cut-services.md) | TASK-122 (R-13) — **RULED 2026-07-14: cut fireflies/reddit, finish the metabase cut, cut method+notion credentials — EXECUTED, `completed`** | Configure or cut: fireflies, reddit, metabase, method? |
...
| DEC-8 | [`DEC-8-first-removal-review.md`](DEC-8-first-removal-review.md) | TASK-100 (B-17), fed by TASK-128 (W-6) — **RULED 2026-07-14: cut `cpa`/`cs`, hold+investigate `kc`, cut design-system build-phase docs — EXECUTED, `completed`** | The first mechanical removal review nominates agents `cpa`/`cs`/`kc` and knowledge area `04-shared-systems` — keep, cut, or (for `kc`) investigate a measurement caveat first? |
| DEC-9 | [`DEC-9-delete-or-defend-list.md`](DEC-9-delete-or-defend-list.md) | TASK-100 (B-17) — **RULED 2026-07-14: execute per DEC-4/DEC-5/DEC-8 + archive the 3 dormant repos — EXECUTED, `completed`** | The consolidated **per-product** delete-or-defend list (all 17 `ECOSYSTEM.md` products + shared systems + dormant repos + the 22 CLI service groups) — cites DEC-4/DEC-5/DEC-8 rather than duplicating them, and adds one new orphan-credential finding (`notion`, same shape as DEC-5's `metabase`/`method`). |
...
**DEC-4, DEC-5, DEC-8, DEC-9 are ruled and executed** (2026-07-14; see each
memo's header for the exact commits). TASK-118, TASK-122, TASK-100, and
TASK-120 (R-11's scope this ruling unblocked) are `completed` in Task
Copilot. DEC-8's `kc` nomination was explicitly **not** cut — held for a
proposed collector fix, per the ruling's own instruction — and remains
open, not "ruled cut."

DEC-7 is still `blocked` in Task Copilot — the honest status for
"preparable work complete, ruling not made yet," untouched by this pass.
