...
> **RULED 2026-07-14: amend, per-class ratchet at measured reality — EXECUTED (`claude-copilot@9359505`), TASK-112 `completed`.** **CORRECTED same day** (`claude-copilot@b50f66a`, `copilot-control-tower@34e746c`): the first cut's bar-equals-median shape was unfalsifiable (a pass by construction, per `t2`). Restructured into a falsifiable CEILING (fails if a class's median exceeds it — confirmed live: 5 classes already breach the shared default ceiling), a pre-registered descend-only ratchet (next step: a quarter reduction per class), and a separate, honestly-failing aspiration claim (`framework-agent-return-aspiration`, ~100 tokens). See [`DEC-1`](DEC-1-agent-return-bar.md) §7 for the full correction and the PROPOSED REGISTER PATCHES.
