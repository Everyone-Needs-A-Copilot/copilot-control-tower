...
|---|---|---|---|
| B-1 | The `cc` control-plane CLI and the `copilot` service CLI installed at supported absolute paths. | The test installer installs immutable compatible commits without relying on bare `cc`; the native release app resolves a signed, checksum-pinned `cc` from its own Resources directory. | **Signed helper published and vendored.** Universal `cc` 1.7.2 is published from Claude foundation `v5.13.2`, pinned by SHA-256, and release-gated against its Apple notarization record, a live GitHub device-flow HTTPS probe, a Finder-equivalent minimal-`PATH` organization-discovery probe, and the app compatibility floor. The existing-account live install still verifies the separately installed `copilot` service CLI. |
...
| B-5 | Scoped Infisical credentials in the laptop Keychain. | **Corrected 2026-08-02 (WP-395 G-7):** no `copilot infisical identity provision` subcommand exists or ever has — real `copilot infisical identity` exposes only `list`/`create`, and `create` mints a bare identity object with no universal-auth method, client secret, project membership, or role. The onboarding `secret-store` stage (`onboard.py`'s `_provision_store`) only confirms the org's shared store is already reachable with credentials this Mac already has; it provisions nothing new and writes nothing to Keychain. | **Never built.** Per-device Infisical credential provisioning/delivery is an open design gap, not a built capability — see WP-395 G-5 (no per-device store identity provisioning) and G-6 (no delivery mechanism; needs an ADR before code). |
...
| B-8 | The signed user app installed (optional for the CLI proof, required for the product). | The native exact-ref release pipeline Developer-ID signs, notarizes, staples, and Gatekeeper-verifies the app and drag-install DMG. | **Finder-safe complete-helper release ready for user handoff.** The final app embeds `cc` 1.7.2 unchanged and passed Apple notarization, stapling, Gatekeeper assessment, mounted-DMG inspection, the 110-check native scenario matrix, and a final-bundle onboarding plan under Finder-equivalent environment variables. Live onboarding remains the user's next action. |
...
The independently signed universal `cc` artifact is now published and pinned.
The native app resolves it from `Contents/Resources/cc`; CI and the release
packager verify its checksum, upstream signature, notarization record, and
compatibility floor without re-signing it. The remaining proof is the live
onboarding transaction on the user's own account and machine.
...
| CLI install on a new machine | **Signed native helper published and vendored** | Universal `cc` 1.7.2 is pinned by SHA-256 with upstream signing/notarization evidence, a live device-flow HTTPS probe, and a Finder-equivalent organization-discovery probe. Immutable source commits, plan/apply, collision hold, bundle-relative discovery, and verify-not-resign gating are wired. |
| Machine-identity bootstrap-cred provisioning | **Corrected 2026-08-02 (WP-395 G-7): Never built.** `copilot infisical identity provision` has never existed as a `copilot infisical` subcommand (only `identity list`/`create` exist); `identity create` returns a bare identity with no universal-auth method, client secret, project membership, or role. | The onboarding `secret-store` stage confirms pre-existing store access only, never mints or delivers a credential. Per-device provisioning/delivery is an open design gap (WP-395 G-5/G-6 — needs an ADR before code). |
...
| Native user app (wizard + tray, de-mocked) | **Corrected complete-helper DMG ready and real-pixel verified** | The real vendored helper is embedded unchanged; both the explicit pre-code loading state and the settled GitHub device-code screen were verified from the notarized build. Live account onboarding remains. |
