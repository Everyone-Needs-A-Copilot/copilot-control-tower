...
| B-8 | The signed user app installed (optional for the CLI proof, required for the product). | The native exact-ref release pipeline Developer-ID signs, notarizes, staples, and Gatekeeper-verifies the app and drag-install DMG. | **TLS-corrected complete-helper release ready for user handoff.** The final app embeds `cc` 1.7.1 unchanged and passed Apple notarization, stapling, Gatekeeper assessment, mounted-DMG inspection, the 110-check native scenario matrix, and a real-pixel device-code proof. Live onboarding remains the user's next action. |
...
| CLI install on a new machine | **Signed native helper published and vendored** | Universal `cc` 1.7.1 is pinned by SHA-256 with upstream signing/notarization evidence and a live device-flow HTTPS release probe. Immutable source commits, plan/apply, collision hold, bundle-relative discovery, and verify-not-resign gating are wired. |
