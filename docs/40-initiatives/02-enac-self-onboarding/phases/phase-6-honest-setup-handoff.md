...
3. **Completed — package a current signed/notarized User-app release candidate.** The corrected arm64 DMG from Control Tower source `0026e17` embeds the notarized universal `cc` 1.7.2 helper from foundation `v5.13.2`. It passed the helper's live GitHub device-flow HTTPS probe, a Finder-equivalent minimal-`PATH` organization-discovery probe, Apple's notarization service, stapler, code-signing, Gatekeeper, mounted-DMG inspection, and a real-pixel device-code proof. The earlier `cc` 1.7.0 and 1.7.1 DMGs are superseded: 1.7.0's frozen runtime omitted the CA trust bundle, while 1.7.1 could not resolve Homebrew's `gh` from a Finder-launched app.
