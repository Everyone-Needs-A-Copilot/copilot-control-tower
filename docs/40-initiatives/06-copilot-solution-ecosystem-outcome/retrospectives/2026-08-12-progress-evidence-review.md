...
Evidence window: current task graph and work products through WP-826
...
- The repository-contained Control Tower launcher and later self-authorizing variants were rejected. Architecture WP-815 confirms that a fully automated first install is impossible under this threat model without an existing independent trust anchor. Local source WP-822 now confines the checkout to deterministic unsigned preparation and exact external tuple verification; QA WP-824 and security WP-826 approve that exact source-only boundary. The first secure install must still be owner-attended, independently signed, root-staged, and bound to one exact package/source tuple out of band. No app candidate exists.
...
- The formerly red `claude-copilot` workflow failures have local fixes. WP-825 combines the PR #63 release-trust work and reviewed PR #65 Knowledge-runtime work at signed local commit `f3a3bc9`; QA WP-820 and security WP-823 approve that exact local integration, including 31 of 31 external Actions pinned to full SHAs. It is not pushed, reviewed, merged, or hosted-verified while the owner-directed GitHub pause remains active.
- The reviewed worktrees are clean and committed. GitHub Actions remain disabled on all 55 organization repositories; no queued or running jobs exist.
...
| Active | TASK-287, 297 | Local source convergence is approved; candidate production and signed shared-content release remain blocked by external first-trust, provider, hosted, or human-review gates. |
...
- WP-822 supersedes the rejected source boundary at local public commit `6d21161`, tree `7625c7a`. The mutable checkout can now prepare only deterministic unsigned inputs and an intentionally incomplete approval template. Exact integer schema enforcement rejects boolean, string, floating, null, array, and object confusion; an independently supplied manifest binds the immutable source, monotonic package version, application identity and digests, and final signed-package SHA-256. Same-team package substitution, downgrade, source drift, hostile startup/Git state, unsafe trees, writable ancestry, symlinks, ACLs, and ownership failures reject locally.
...
- WP-822 remains local, clean, and unpushed under the organization-wide GitHub pause. The older `/Applications` anchor remains rejected.
- **Current result:** the exact local source-only boundary has fresh QA and security approval, but no independently authorized package ceremony has occurred, no protected publisher anchor is installed, no Control Tower candidate has been built, and nothing has been published.
...
- Protected per-read Knowledge provenance was independently approved at its earlier exact revision by WP-780/WP-782. WP-825 now integrates that reviewed runtime with PR #63 on signed local commit `f3a3bc9`, tree `7b74d4e`. QA WP-820 approves 2,664 portable tests with 11 skipped and 46 machine-marked tests deliberately excluded; security WP-823 approves the signer, provenance, mutable-tamper, entitlement-race, path, cleanup, secret, and supply-chain boundaries. The integrated workflow tree has 31 of 31 external Actions pinned to full SHAs with no conflicting refs.
- The integrated branch and related reviewed worktrees are clean. It has not been pushed, reviewed, merged, tagged, or exercised in hosted CI while Actions remain disabled.
- **Current result:** the accounting current release slice and combined framework source are locally QA/security approved, but TASK-297 cannot complete until protected hosted review/merge, organization review/tag/pin, GitHub Support garbage collection, the named need-to-have access review, exact entitlement-qualified live proof, and narrowly restored hosted checks.

### `claude-copilot` release trust — PR #63

- PR #63 remains open, review-required, and blocked at its hosted head. Its hosted failures are stale results from the short-lived workflow restore window, not current local proof.
- The dependency-ordered local convergence is complete in WP-825 at signed commit `f3a3bc9`. QA WP-820 and security WP-823 approve that exact clean local integration; machine-marked and hosted checks remain outside those verdicts.
...
- Hosted checks are intentionally paused while Actions maintenance is active. The local integration is unpushed and the PR remains `REVIEW_REQUIRED` and unmerged; stale hosted failures must not be used as either approval or rejection of the converged head.
...
| AC-15 | External first-trust gate | WP-815 architecture; WP-822 local source; QA WP-824; security WP-826 | Provision an independent Installer signing identity and owner-approve the exact package/source tuple; implement and review root-staged monotonic ceremony; attended install and installed-state evidence; then candidate |
...
| AC-23 | Active gate | Owner approval, signed accounting root/tag, protections, signed organization candidate; integrated source WP-825 with QA WP-820/security WP-823 | Protected hosted reviews/merges/checks, provider GC, organization tag/pin, exact entitled proof, access confirmation |
...
| **TASK-287: This task is to finish crash-only supervision and produce a verified Control Tower release candidate without publishing it.** | Local source WP-822 has QA WP-824 and security WP-826 approval. It still requires an independently controlled Developer ID Installer identity, owner approval of one exact source/package tuple through a trusted outside channel, root-owned staging, a monotonic anti-downgrade ledger, and an attended first-install ceremony under WP-815. | Immutable pushed source; exact package/source binding; valid same-team substitution, downgrade, and race tests; clean-Quit and crash-restart proof; signed, notarized, stapled candidate; installed receipt and identity verification; fresh ceremony/candidate QA and security approval. |
...
| **TASK-297: This task is to approve and publish the organization and accounting content as signed, immutable inputs.** | Local integrated source WP-825 has exact-commit QA WP-820 and security WP-823 approval. It still requires protected hosted PR reviews/merges, GitHub Support purge/GC confirmation, final need-to-have roster confirmation, organization review/tag/pin, and narrowly restored hosted checks. | Reviewed merges; new nonmoving signed tags; exact manifest pins and signer-policy proof; provider confirmation that the legacy object no longer resolves; approved least-privilege roster; exact entitlement-qualified consumer identities; and green hosted checks on the reviewed heads. |
...
1. Preserve clean local integration WP-825 and its exact QA WP-820/security WP-823 verdicts while the organization-wide GitHub pause remains active; do not broaden those verdicts to hosted or machine-marked proof.
...
3. Obtain owner approval for that exact tuple, implement and review root-owned staging plus the monotonic version-floor ledger, then obtain fresh ceremony/candidate QA and security beyond WP-824/WP-826's source-only scope.
