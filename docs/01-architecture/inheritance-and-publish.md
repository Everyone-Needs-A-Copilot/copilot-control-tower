...
2. **Open a clone of the target tier repo.** For example, an org may use `<host>-copilot-internal` and a department may use `<host>-copilot-<department>`. Each product (`claude`, `cli`, `codex`, `knowledge`) has its own set of tier repos — elevate into the repo matching the product the item belongs to.
3. **Copy the item into the tier repo's matching top-level dimension directory**, preserving the same shape it has under the project's `.claude/`, at the tier repo's root (not nested under `.claude/`):
   ```
   cd /path/to/<host>-copilot-internal
   mkdir -p skills
...
>
> A prior internal incident demonstrated why this boundary is structural: a clean human-owned authoring checkout was linked into a materialization target, so routine reconciliation treated it like disposable output and removed owned paths. **Never rely on a dirty-tree guard to make this safe.** Elevation is always **copy → commit → push into the tier repo's own working directory** — never a symlink, bind mount, or other path that lets a materializer write through to a human-owned checkout.

Cross-references: ADR-008 (formal deferral of `cc publish`); the 2026-08-01 inheritance-readiness audit `WP-372` Case 6 (source of the manual-path discovery) and its P0 section (source of the incident above).
