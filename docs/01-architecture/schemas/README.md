...
| `workspaces.schema.json` | `cc workspace --all --json` / `cc workspace configure --project <path>\|--apply-all --json` / `cc workspace revert --project <path> --json` | Bounded Git-workspace discovery, portable project declarations, opaque personal-project association, bulk configure, and undo. A declaration is never accepted as installation proof. Per-workspace `setup_policy`/`can_apply_now`/`undo` let the app choose checkbox-vs-immediate-Add grammar without computing it; `setup_policy: "automatic"` is reserved (not yet emitted -- needs a "known projects as of grant time" ledger this slice does not build). |
