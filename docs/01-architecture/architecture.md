...
- **Vendored CLI binaries are a cross-repo signing contract** *(fixes B-C2, B-H1):* `claude-copilot` CI publishes `copilot`/`cc` as **already-signed, notarized, universal** artifacts at a **pinned SHA+version**; Control Tower CI verifies the pinned SHA, universal slices, upstream Developer ID signature, accepted notarization record, and compatibility floor, and never re-signs the helper. Apple creates tickets for standalone binaries but does not support stapling those tickets to the raw file, so offline `spctl`/Gatekeeper proof occurs at the final nested-code boundary: the signed helper is embedded unchanged, then the Control Tower app and DMG are notarized, stapled, and assessed. The `.pkg` postinstall strips quarantine; a `cli-spawnable` doctor check surfaces a Gatekeeper kill as a named finding, not a generic red.
