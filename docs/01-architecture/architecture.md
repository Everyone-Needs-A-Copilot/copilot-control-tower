...
**Security-sensitive keys are honored ONLY from the managed (forced) domain** *(fixes B-C5, the supply-chain preference-write attack):* `UpdateFeedURL`, `FoundationMirror`, `EcosystemSeedURL`, `HTTPSProxy`, `GitHubHost`, `AuthMode`, `AllowSelfUpdate`, `Deprovisioned`, `AdminContact` are read via `CFPreferencesAppValueIsForced`; a value present only in the user domain is **ignored** in favor of the compiled-in default, and logged as a tamper event. (`AdminContact` is forced-domain-only so a local user cannot redirect IT safety escalations — closes the threat-model finding that would otherwise silently defeat A-C5; see [`../05-security/threat-model.md`](../05-security/threat-model.md).) On unmanaged machines the compiled-in trust root (minisign pubkey + default feed) is authoritative — trust roots are code, not config.
