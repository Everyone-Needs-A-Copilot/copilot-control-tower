...
| `cc reconcile assess --json` / `cc reconcile assistant-prepare --request <private-json> --json` / `cc reconcile assistant-run --session-id <opaque-id> [--json]` / `cc reconcile assistant-status --session-id <opaque-id> --json` / `cc reconcile plan --request <same-private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(cc 2.8.0+; response schema 2.0, request schema 1.0)** | One of the strict assess, assistant-prepare, assistant-run, assistant-status, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Explicit selection and the optional opaque `assistant_proposal_id` use [`schemas/reconcile-request.schema.json`](schemas/reconcile-request.schema.json). Assess returns typed repository `scope`, exact component-level `default_selection` and `assistant_selection`, `scope_counts`, `batch_summary`, and `resolution_summary` with `managed_separately` plus exact `left_unchanged` reason buckets. Assistant lifecycle reports include Python-authored `progress` stage and liveness. | This is the shipping Control Tower project workflow. Python owns repository-scope proof, complete machine/project assessment, component-level authorization, both selection lists, every count/reason, assistant progress and stale-liveness verdicts, bounded candidate construction, proposal validation, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, fresh verification, explanations, and diagnostics. Swift displays those facts, defaults to the union of Python's two authorized lists, and may only remove whole projects. It never widens component scope or reconstructs eligibility, totals, reasons, or liveness. For assistant work it creates the exact private request, opens the exact bundled helper in Terminal with only an opaque session id, polls typed status, and attaches the returned opaque proposal id to that same request. Python still produces the exact plan and is the only component that applies or verifies project changes. Exit `0` is a successful command, exit `1` is a renderable blocked/partial/action-required business result, and exit `2` is a structured error. A consumed or stale plan/proposal never causes automatic replacement-plan apply. Diagnostics are redacted atomic mode-`0600` files below a mode-`0700` owned boundary with bounded retention. |
