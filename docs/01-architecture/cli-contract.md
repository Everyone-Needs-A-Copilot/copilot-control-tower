...
> **Freeze status & source of truth (refreshed 2026-08-04).** The authoritative contracts are implemented upstream in `claude-copilot/tools/cc`. The helper now includes the project-centered `workspace` surface and the unified Python-owned `reconcile` workflow alongside `auth`/`doctor`/`layers`/`freshness`/`update`/`projects`/`onboard`. There is no standalone `repair` or `publish` verb, and none is scheduled: Git history remediation lives inside `cc onboard` routing, while project ecosystem repair lives inside the typed `cc reconcile` transaction. Standalone `repair`/`publish` remain **formally deferred** — see `ADR-008` and the "Deferred verbs" section below. This prose and the [`schemas/`](schemas/) in this repo encode the contracts; schemas win for machines.
...
Full detail: `../03-design/design-integration.md` §1. Rationale: [`architecture.md`](architecture.md) §6.
...
| `cc reconcile assess --json` / `cc reconcile prepare --json` / `cc reconcile guide-prepare --request <private-json> --json` / `cc reconcile guide-start --guide-id <opaque-id> --assistant <codex\|claude-code> --json` / `cc reconcile guide-check --guide-id <opaque-id> --project <exact-path> --json` / `cc reconcile guide-status --guide-id <opaque-id> --json` / `cc reconcile guide-finalize --guide-id <opaque-id> --json` / `cc reconcile plan --request <same-private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(active preparation cc 2.10.0+; response schema 2.0, request schema 1.0)** | One of the strict assess, prepare, guide, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Prepare returns completed local checkpoints, download-only shared refreshes, GitHub authority evidence, holds, and a nested fresh assessment. Assess returns typed repository `scope`, exact component-level `default_selection` and `assistant_selection`, `scope_counts`, `batch_summary`, and `resolution_summary` with exact reason buckets. Guide reports identify the immutable instruction and project files, Python's short `start_prompt`, the least set of workspace roots needed by the batch, Python-owned progress, and one exact status/reason list for every selected project. | This is the shipping Control Tower project workflow. Python owns repository-scope proof, Product-project checkpoint eligibility, shared-repository download-only refresh, GitHub authority evidence, complete machine/project assessment, component authorization, selection, every count/reason, the immutable work order and copy prompt, project verification, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, explanations, and diagnostics. Setup never pushes, never checkpoints ecosystem repositories, and never upgrades shared setup beyond download-only; `WRITE`, `MAINTAIN`, or `ADMIN` evidence belongs to a separate explicit authoring workflow. Swift renders those facts and never runs Git. Exit `0` is a successful command, exit `1` is a renderable blocked/partial business result, and exit `2` is a structured error. Diagnostics and guide state are private atomic files under owned mode-`0700` boundaries. |
| `cc workspace migrate (--all\|--project <path>) [--plan-id <opaque-id>] [--apply] --json` **(legacy helper compatibility; cc 2.5.0+, schema 1.1 in cc 2.5.3)** | `{schema_version,mode(plan\|apply),result,plan_id,summary:{eligible,held,residual-guidance,total_guided},candidates:[…],ledger:[…],apply_summary?,after?,diagnostics?}` (see [`schemas/workspace-migrations.schema.json`](schemas/workspace-migrations.schema.json)) | Retained for command-line/backward compatibility, but no longer consumed by the native app. Its two legacy deterministic migration families were the evidence base for `cc reconcile`; new Control Tower project work uses the unified contract above so every selected project has one explicit route and every primary-state count partitions the census. |
...
| `cc connections --json` **(stage B, task 221)** | `{schema_version,result(ok\|copilot-unavailable\|org-config-unavailable),detail,org,store:{type,reachable,scope,detail,diagnostic},connections:[{id,name,description,tier,mode,requires_secret,store_scope,secret_state(ready\|needs-connect\|no-store),missing}]}` (see [`schemas/connections.schema.json`](schemas/connections.schema.json)) | Closes the "Your connections" empty-state gap (task 221 investigation, WP-388/389/390): shells `copilot --json layers` (cli-copilot foundation ≥v0.3.2, which now carries `requires_secret`/`store_scope` per service) and, for each service's `store`/`any`-hinted secret names, presence-checks them (never values) against the org's shared Infisical store — one `copilot infisical --json secret list` call per run, never per name; `keychain`-hinted names (plus any structurally never-store-able bootstrap name, e.g. the Infisical machine identity, regardless of its declared hint — WP-395 G-1) are instead presence-checked against the local OS keychain. Read-only, no lock. `secret_state` is the only field the app filters connections on (READY TO USE vs. AVAILABLE TO CONNECT); `result` is a separate envelope-level diagnostic for two distinct fail-closed cases (`copilot` unresolvable vs. the inherited org config not yet materialized) — `org-config-unavailable` still returns the full roster with every store-dependent row honestly marked `no-store`, rather than an empty list. `store.detail` is always a plain-language, `cc`-authored sentence — never a raw passthrough of an underlying command's stderr (WP-395 G-2); the additive `store.diagnostic` carries that raw text for logs only, never for user-facing rendering. |
| `cc connect <service-id> [--check] --json` **(WP-395 D-6, task 222)** | `{schema_version,result(ok\|unknown-service\|invalid-input\|copilot-unavailable\|org-config-unavailable),detail,mode(connect\|check),service:<connections-row>\|null,credentials:[{name,outcome(stored\|already-present\|failed),detail}]\|null}` (see [`schemas/connect.schema.json`](schemas/connect.schema.json)) | The in-app secret-input connect mechanism the owner ratified (never `.env`; a future device-flow/OIDC North Star model remains separate and unbuilt, WP-396). Reuses `cc connections`' own service-roster/presence-check machinery to find one service's currently-missing credential NAMES, reads their VALUES from **stdin only** as a JSON object `{"<NAME>":"<value>",...}` — never argv, an environment variable, or a file (see `connect.py`'s module docstring for why) — and writes each to the OS keychain (service `copilot-cli`, account = the NAME) via a non-leaking `security -i` batch-mode invocation (never `-w <value>` on argv, which `ps` would expose) before re-running the presence checks and returning the fresh row. `--check` skips stdin and the write entirely, re-evaluating and returning only the row (the app's post-connect refresh). Never echoes a value in any field; an empty/non-string/line-break-containing value is rejected before ever reaching the keychain writer with a structured, value-free `failed` outcome. A partial failure (some names stored, others not) still reports `result: "ok"` with an accurate per-name `credentials[]` — a consumer that needs one success/failure signal checks whether any entry's `outcome` is `"failed"`. `unknown-service` and a service with zero missing credentials (which never reads stdin at all) are both ordinary, fully-structured outcomes, never an error page. No lock (touches only the per-user OS keychain, not layer/manifest state). |

## `cc reconcile` one-conversation guided setup (cc 2.9.1)

The guided lifecycle is the default human-facing route for a large project
folder:

1. Swift sends Python the exact union of its `default_selection` and
   `assistant_selection`, minus only whole projects the person unchecked.
   Python re-assesses before accepting it. Dirty, held, unsafe, ambiguous, and
   verified ecosystem repositories never enter the package.
2. `guide-prepare` writes `INSTRUCTIONS.md` and `PROJECTS.json` once, mode
   `0400`, below `.copilot-control-tower/reconciliation/<guide-id>/` in the
   first selected workspace root. Private mode-`0600` state stores their
   fingerprints. Only roots containing selected projects are returned for
   assistant access.
3. `guide-prepare` also returns Python's short `start_prompt`, which points to
   the exact instruction path. Control Tower opens one plain Terminal at the
   returned projects root and runs only `cd <root>`. The person starts Claude
   Code or Codex, copies and pastes that prompt, stays in this one
   conversation, and can ask questions there. Control Tower never starts or
   supervises the assistant and never sends the person through one session per
   project.
4. The work order requires the assistant to read each project's own
   instructions, preserve project-owned behavior, respect dossier targets and
   stop conditions, and avoid commits, pushes, resets, cleans, stashes,
   unrelated deletion, secrets, or dirty projects.
5. `guide-check` runs fresh verification for one exact project and updates its
   Python-owned status and plain-language reasons. `guide-status` is read-only.
   `guide-finalize` freshly verifies the whole batch. Only those Python reports
   can turn a project green; the app never infers success from the Terminal
   process or assistant output.

The earlier bounded recipe-selection lifecycle remains available as a
restricted deployment option, but it is not the default app route.

## `cc reconcile` bounded Claude Code preparation (cc 2.8.0)

The assistant lifecycle extends reconciliation planning; it is not a second
mutation engine:

1. `assess` emits two disjoint Python-authorized lists. `default_selection`
   already has deterministic recipe bindings. `assistant_selection` contains
   customized projects for which Python can construct a closed candidate set.
   Each selection preserves Python's exact Claude-only, Codex-only, or combined
   component scope. `resolution_summary` separately reports automatic,
   Claude-assisted, total actionable, managed-separately, exact unchanged
   reason, new-setup, and correction counts. Verified Copilot ecosystem
   repositories use their own scope and never enter either selection list.
   Swift displays these lists and counts; it does not reconstruct eligibility,
   scope, reasons, or totals.
2. `assistant-prepare` accepts the same owner-only request used for planning,
   re-inspects the selected projects, refuses an already-attached proposal,
   and creates one private expiring session. Its report returns only an opaque
   `session_id`, expiry, the exact selected paths for identity checking,
   Python-authored progress/liveness, and next actions.
3. `assistant-run` claims that session once. Control Tower launches the exact
   bundled `cc` executable in visible Terminal with only `reconcile
   assistant-run --session-id <opaque-id>`; no project path or prompt is placed
   on the Terminal command line. The helper invokes a protected Claude Code
   executable in a private `0700` working directory with a constrained
   environment, safe/plan-only settings, no tools, no project working tree,
   and no session persistence.
4. Claude Code receives a content-free selection packet: opaque project and
   candidate identifiers, component groups, bounded evidence codes, and the
   rule to choose exactly one offered candidate per group. It does not receive
   project paths or project-authored content and has no authority to author a
   command, path, file content, patch, recipe operation, or success verdict.
   Duplicate, unknown, incomplete, malformed, oversized, or prohibited output
   rejects the session without project mutation.
5. `assistant-status` returns `running`, `ready`, or `blocked` plus one factual
   progress stage (`session-prepared`, `claude-code-running`, Python validation,
   `ready`, or `blocked`) and liveness (`waiting`, `active`, `stale`, `complete`,
   or `blocked`). Python alone derives stale liveness from its heartbeat; Swift
   displays it without a percentage, timer, or independent timeout verdict.
   `ready` carries
   one opaque `proposal_id` only after Python has resolved the offered choices,
   revalidated the resulting typed recipes, and bound them to the exact base
   request. Swift rejects session or selected-path identity drift, attaches the
   id as `assistant_proposal_id`, and continues through the ordinary
   plan/review/apply/verify calls. Proposal expiry, reuse, stale evidence,
   changed request authority, dirty work, unsafe paths, or concurrent claims
   fail closed. Claude Code never writes a selected project; Python remains the
   sole planner, writer, rollback authority, and verifier.

## Deferred verbs

No CLI verb inventory above may list `repair` or `publish` — neither exists in `claude-copilot/tools/cc/src/cc/commands/`. Their scope was explicitly deferred by the project’s architecture decisions:

- **`repair`.** History remediation now lives inside `cc onboard`'s own routing rather than a standalone verb: its closed 8-state classifier routes fast-forwardable rows to an in-onboard fast-forward repair and every review-state (dirty, ahead-only, divergent-identical, divergent-different, wrong-origin, unreadable) to the owner. A first-class `cc repair` verb is deferred; none is scheduled.
...
Added to close CSE open question 2 / decision D7.1 (`cse-alignment-decisions.md`): the contract has status/sync verbs (`doctor`/`update`/`resolve`/…) but, until now, no way for a user to **discover** which department/org layers they are entitled to, or to **join** one. This is a **proposed** addition, the same not-yet-upstream status as `copilot publish --json` above, to be folded into upstream WS-A scope at freeze. The prior design *descoped* department discovery entirely; this reverses that descope.
...
- **`flock` on `copilot.lock`** across `update` / `onboard` / `deprovision`; fail-fast if held. A **global per-host mutex across all verbs** lets `deprovision` drain pending syncs before wiping. *The CLI self-serializes — Control Tower is not the lock.* There is no standalone `repair` verb to serialize; `onboard`'s in-transaction history repair (row `action: repair`) is covered by `onboard`'s own lock.
