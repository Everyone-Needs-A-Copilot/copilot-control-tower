...
| `cc workspace migrate (--all\|--project <path>) [--plan-id <opaque-id>] [--apply] --json` **(cc 2.5.0+; schema 1.1 in cc 2.5.3)** | `{schema_version,mode(plan\|apply),result,plan_id,summary:{eligible,held,residual-guidance,total_guided},candidates:[…],ledger:[…],apply_summary?:{…,updated_still_guided,failed_still_guided,detail},after?,diagnostics?}` (see [`schemas/workspace-migrations.schema.json`](schemas/workspace-migrations.schema.json)) | The CLI owns the full migration census, exact-plan freshness check, file operations, rollback, independent verification, completed-action ledger, overlap explanation, and durable diagnostic record. Plan mode is read-only. Apply requires the opaque current `plan_id`; a stale plan re-inspects every project and returns `blocked` without applying the replacement plan. Each Apply saves one atomic, redacted, mode-`0600` JSON record below `~/.claude/cc/diagnostics/workspace-migrations/`, retains the newest 20 owned records, and reports an explicit unavailable reference if persistence fails. Records include preflight state, bounded target fingerprints, completed operations, component-level verifier evidence, exception classification, and per-target rollback outcomes; they never include file contents, environment variables, stdin, credentials, or raw subprocess streams. Control Tower renders the CLI-authored cohorts, overlap sentence, ledger, and diagnostic reference; it never derives eligibility, performs project filesystem work, or constructs a migration itself. |
