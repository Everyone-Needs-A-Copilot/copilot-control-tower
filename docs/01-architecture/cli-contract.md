...
| `cc reconcile assess --json` / `cc reconcile plan --request <private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(cc 2.6.0+; schema 1.0)** | One of the strict assess, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Explicit selection uses [`schemas/reconcile-request.schema.json`](schemas/reconcile-request.schema.json). | This is the shipping Control Tower project workflow. Python owns complete machine assessment, project census, component recommendations, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, fresh verification, counts, explanations, and diagnostics. Swift writes only the person's explicit roots/projects/components/recipe choices to an owner-only request, preserves the same bytes across plan/apply/verify, passes the opaque plan id, and renders returned truth. Exit `0` is a successful command, exit `1` is a renderable blocked/partial/action-required business result, and exit `2` is a structured error. A consumed or stale plan never causes automatic replacement-plan apply. Diagnostics are redacted atomic mode-`0600` files below a mode-`0700` owned boundary with bounded retention. |
