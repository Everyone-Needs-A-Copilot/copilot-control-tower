...
| `cc reconcile assess --json` / `cc reconcile plan --request <private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(cc 2.6.1+ for default batching; schema 1.0)** | One of the strict assess, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Explicit selection uses [`schemas/reconcile-request.schema.json`](schemas/reconcile-request.schema.json). Assess returns `machine_summary`, `batch_summary`, and `default_selection`; every default selection contains both components and any deterministic reviewed recipe binding. | This is the shipping Control Tower project workflow. Python owns complete machine assessment, the two-pass universal Claude-plus-Codex safety check, project census, batch categories and counts, default selection, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, fresh verification, explanations, and diagnostics. Swift starts from Python's complete safe batch and may only remove projects in response to project-level checkbox input. It cannot add an ineligible project, choose one copilot, or choose a recipe. It writes the exact resulting roots/projects/components/recipe bindings to an owner-only request, preserves the same bytes across plan/apply/verify, passes the opaque plan id, and renders returned truth. Exit `0` is a successful command, exit `1` is a renderable blocked/partial/action-required business result, and exit `2` is a structured error. A consumed or stale plan never causes automatic replacement-plan apply. Diagnostics are redacted atomic mode-`0600` files below a mode-`0700` owned boundary with bounded retention. |
