...
| `cc reconcile assess --json` / `cc reconcile assistant-prepare --request <private-json> --json` / `cc reconcile assistant-run --session-id <opaque-id> [--json]` / `cc reconcile assistant-status --session-id <opaque-id> --json` / `cc reconcile plan --request <same-private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(cc 2.7.0+ for bounded Claude Code preparation; schema 1.0)** | One of the strict assess, assistant-prepare, assistant-run, assistant-status, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Explicit selection and the optional opaque `assistant_proposal_id` use [`schemas/reconcile-request.schema.json`](schemas/reconcile-request.schema.json). Assess returns `machine_summary`, `batch_summary`, `default_selection`, `assistant_selection`, and `resolution_summary`; both selection lists contain universal Claude-plus-Codex component intent, while only deterministic automatic rows carry recipe bindings before assistant preparation. | This is the shipping Control Tower project workflow. Python owns complete machine assessment, the universal Claude-plus-Codex safety check, both authorized selection lists, new-setup/correction and automatic/assisted/held counts, bounded candidate construction, proposal validation, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, fresh verification, explanations, and diagnostics. Swift defaults to the union of Python's two authorized lists and may only remove whole projects. For assistant work it creates the exact private request, opens the exact bundled helper in Terminal with only an opaque session id, polls typed status, and attaches the returned opaque proposal id to that same request. Python still produces the exact plan and is the only component that applies or verifies project changes. Exit `0` is a successful command, exit `1` is a renderable blocked/partial/action-required business result, and exit `2` is a structured error. A consumed or stale plan/proposal never causes automatic replacement-plan apply. Diagnostics are redacted atomic mode-`0600` files below a mode-`0700` owned boundary with bounded retention. |
...
## `cc reconcile` bounded Claude Code preparation (cc 2.7.0)
...
1. `assess` emits two disjoint Python-authorized lists. `default_selection`
   ...
   `resolution_summary` separately reports automatic, Claude-assisted, held,
   total actionable, new-setup, and correction counts. Swift displays these
   lists and counts; it does not reconstruct eligibility or reconcile totals.
2. `assistant-prepare` accepts the same owner-only request used for planning,
   ...
   `session_id`, expiry, the exact selected paths for identity checking, and
   next actions.
...
5. `assistant-status` returns `running`, `ready`, or `blocked`. `ready` carries
