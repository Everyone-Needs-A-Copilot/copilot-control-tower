...
| `cc reconcile assess --json` / `cc reconcile guide-prepare --request <private-json> --json` / `cc reconcile guide-start --guide-id <opaque-id> --assistant <codex\|claude-code> --json` / `cc reconcile guide-check --guide-id <opaque-id> --project <exact-path> --json` / `cc reconcile guide-status --guide-id <opaque-id> --json` / `cc reconcile guide-finalize --guide-id <opaque-id> --json` / `cc reconcile plan --request <same-private-json> --json` / `cc reconcile apply --request <same-private-json> --plan-id <opaque-id> --json` / `cc reconcile verify --request <same-private-json> --json` / `cc reconcile recover --json` **(guided lifecycle cc 2.9.1+; response schema 2.0, request schema 1.0)** | One of the strict assess, guide, plan, apply, verify, recover, or structured-error branches in [`schemas/reconcile.schema.json`](schemas/reconcile.schema.json). Assess returns typed repository `scope`, exact component-level `default_selection` and `assistant_selection`, `scope_counts`, `batch_summary`, and `resolution_summary` with `managed_separately` plus exact `left_unchanged` reason buckets. Guide reports identify the immutable instruction and project files, Python's short `start_prompt`, the least set of workspace roots needed by the batch, Python-owned progress, and one exact status/reason list for every selected project. | This is the shipping Control Tower project workflow. Python owns repository-scope proof, complete machine/project assessment, component authorization, selection, every count/reason, the immutable work order and copy prompt, project verification, typed recipes, plan fingerprints, locking, containment, snapshots, mutation, rollback, explanations, and diagnostics. Swift displays those facts, may only remove whole projects, and opens a plain Terminal at the returned projects root. The person starts Claude Code or Codex and pastes Python's prompt themselves; Swift does not start, supply input to, supervise, poll, or infer progress from an assistant. The assistant may inspect and correct only the exact selected product projects; ecosystem repositories, unsafe projects, and unused watched roots are excluded. An assistant statement is never proof: only `guide-check` or `guide-finalize` can mark a project ready. Exit `0` is a successful command, exit `1` is a renderable blocked/partial business result, and exit `2` is a structured error. Diagnostics and guide state are private atomic files under owned mode-`0700` boundaries. |
