...
| `cc reconcile run --json` / `cc support latest --json` **(complete setup journey, cc 2.12.6+)** | `reconcile run` emits the versioned 1.x setup-journey verdict with `operational`, `confidence`, the final assessment, summary, and Python-owned diagnostic identity. `support latest` emits the exact 1.x envelope for the newest current-user-owned, mode-`0600` setup report. | Verify invokes only these two contracts. Python checkpoints eligible Product work, refreshes shared repositories download-only, repairs approved project integration, verifies the machine/hierarchy/projects/store, and owns the diagnostic file. Swift validates schema and top-level consistency, then renders the result. It never writes a second report or recomputes readiness. Exit `0` means operational; exit `1` means action required; malformed or inconsistent success-shaped output fails closed. |
