...
| `cc workspace migrate (--all\|--project <path>) [--plan-id <opaque-id>] [--apply] --json` **(cc 2.5.0+)** | `{schema_version,mode(plan\|apply),result,plan_id,summary:{eligible,held,residual-guidance,total_guided},candidates:[…],ledger:[…],apply_summary?,after?}` (see [`schemas/workspace-migrations.schema.json`](schemas/workspace-migrations.schema.json)) | The CLI owns the full migration census, exact-plan freshness check, file operations, rollback, independent verification, and completed-action ledger. Plan mode is read-only. Apply requires the opaque current `plan_id`; a stale plan re-inspects every project and returns `blocked` without applying the replacement plan. Control Tower renders the CLI-authored eligible/held/tailored cohorts and ledger, never derives eligibility or constructs a migration itself. |
