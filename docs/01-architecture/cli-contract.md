...
| Verb | Emits | Notes |
|---|---|---|
| `copilot auth [login\|status] --json` | `{schema_version, kind(device-code\|poll\|status), …}` (see [`schemas/auth.schema.json`](schemas/auth.schema.json)) | GitHub device-flow sign-in. Bare `login` initiates; `login --poll --device-code <code>` performs one poll step; `status` is offline-safe (no network). The OAuth token is written directly to the OS keychain and NEVER appears in this contract (NO-SECRET fitness invariant). |
