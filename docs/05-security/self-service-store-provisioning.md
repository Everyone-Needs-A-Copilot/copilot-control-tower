...
| | |
|---|---|
| **STATUS** | **DESIGN FOR REVIEW — security threat-model pass complete (2026-08-02, @agent-sec, task 222 / WP-399).** Not ratified, not a plan, no code. **Verdict: survives, with changes** — see §11 for the full STRIDE+DREAD pass, the vendor facts §3/§10 flagged as unverified (now confirmed or corrected), and the owner ruling list. Everything this document proposes that does not exist today is marked **[NEW]**; every capability claim about Infisical or GitHub carries a verification status in §3, several of which were upgraded from *Reported* to *Confirmed* by the sec pass. |
