...
| Author git-push credential generation, registration, rotation, revocation (Enhancement B, §6) | **Owns it entirely** — generates the on-device keypair, encrypts the private-key file with a per-machine passphrase stored through the platform's Keychain-backed `ssh-agent`, and registers/rotates only the public key via GitHub's API (itself authenticated via the §1.4 OAuth device-flow) | No role — authorization is GitHub Team membership, a fact about the enterprise's own GitHub org, not a config-domain value | No role — Control Tower never touches the private key, its passphrase, or the registration call |
...
**Generation.** At the moment an author is promoted (`inheritance-and-publish.md` §2.4 — "a provisioning event: add a writable authoring checkout + a write credential"), the CLI generates an ed25519 keypair **on the author's own machine**. The **private key never leaves the machine**. On macOS, OpenSSH writes it as a mode-`0600`, passphrase-encrypted private-key file; the CLI generates a high-entropy per-machine passphrase and `ssh-add --apple-use-keychain` stores that passphrase in the user's login Keychain while loading the identity into `ssh-agent`. This distinction is deliberate: Apple's supported integration stores the **passphrase**, not the private key itself. The key is therefore never a bare plaintext file a working tree or ordinary content scanner can pick up, and neither key nor passphrase is logged. A future Windows re-skin must provide the equivalent encrypted-file + platform-agent/Credential-Manager property before enabling this path there.
...
- **Relationship to §1.4's keychain mechanism** — this is an intentional, justified variant, not an exception: the private key is an encrypted OpenSSH file; its high-entropy passphrase is stored in the OS Keychain and used by the platform-native `ssh-agent`. It is never a bare `~/.ssh/id_ed25519` plaintext file. The encrypted file is the durable identity, Keychain is the passphrase carrier, and the agent is the signing boundary; this is the SSH-specific composition of the same OS secure-storage guarantee §1.4 relies on.
