...
> **Status line — rebuilt from evidence, 2026-08-02.** Describes Copilot Control Tower v0.4.0, a native macOS SwiftUI/AppKit app. The prior version of this document analyzed a Tauri v2 binary with a webview and an IPC bridge. Neither exists in the shipping app — there is no webview, no HTML rendering, and no IPC command allowlist to sandbox. This rewrite re-scopes every boundary to what the native app actually does, drops findings that were specific to the webview architecture, and adds the one native-equivalent finding the retired analysis's own authors flagged as still needed (§2.2 below). Findings are marked **SHIPPED** (the boundary exists in `native/*.swift` today), **NOT SHIPPED / DESIGN POSITION** (the mechanism was designed — sometimes analyzed at length in the retired version of this document — but does not exist in the shipping app, so it is not a live attack surface today), or **CLI-SIDE** (the mechanism is real but lives in the `claude-copilot` repo, out of this document's direct scope).
...
| **Consistency** | Findings below either restate a still-applicable finding from the retired [`redteam-platform.md`](../04-validation/redteam-platform.md) / [`redteam-use-cases.md`](../04-validation/redteam-use-cases.md) (cited by ID), or are marked **NEW (native)** where this re-scope pass found something specific to the native app that the Tauri-era analysis could not have found. |
...
### 2.4 B5 — the launchd watchdog — NOT SHIPPED (G-2)

The retired analysis found a residual (LaunchAgent plist tampering by a same-user attacker, bounded to persistence rather than privilege escalation) in a watchdog mechanism the shipping app does not currently install. **This is not live attack surface today** — there is no `launchd` registration to tamper with. If G-2 is closed (the watchdog gets implemented), re-apply the retired analysis's residual-risk framing (same-user code execution is a precondition this app doesn't introduce or worsen) rather than re-deriving it from scratch.
...
| **#3 — Never-destroy** | Closes any path where the app's own re-materialize behavior could be weaponized against personal content — verified: no resolution/wipe logic exists in `native/*.swift` for an attacker to trigger in the first place. |
