...
```bash
scripts/tests/fixtures/validate.sh
```
...
CT_FIXTURE=needs-attention-codex-dept-fail scripts/tests/fixtures/mock-cc doctor --json

# alias, same meaning
CC_FIXTURE=offline scripts/tests/fixtures/mock-cc doctor --json

# or by flag (wins over either env var)
scripts/tests/fixtures/mock-cc doctor --json --fixture signed-out-claude-personal

# list every available fixture name
CT_FIXTURE=list scripts/tests/fixtures/mock-cc doctor --json

# simulate the env-error / exit-2 path (no trustworthy body at all)
CT_FIXTURE=exit-2 scripts/tests/fixtures/mock-cc doctor --json
```
...
```bash
for f in scripts/tests/fixtures/corpus/*.json scripts/tests/fixtures/invalid/*.json; do
  name="$(basename "${f%.json}")"
  echo "=== $name ==="
  CT_FIXTURE="$name" scripts/tests/fixtures/mock-cc doctor --json | head -c 200
  echo "  (exit=$?)"
done
```
