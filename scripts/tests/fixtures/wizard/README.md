...
(`scripts/tests/fixtures/mock-cc`) — it now serves **both** the M1/M2 doctor corpus
...
scripts/tests/fixtures/mock-cc auth --json

# poll, happy path
CT_AUTH_SCENARIO=authorized scripts/tests/fixtures/mock-cc auth --json --poll

# poll, each honest-failure variant
CT_AUTH_SCENARIO=denied  scripts/tests/fixtures/mock-cc auth --json --poll
CT_AUTH_SCENARIO=expired scripts/tests/fixtures/mock-cc auth --json --poll
CT_AUTH_SCENARIO=timeout scripts/tests/fixtures/mock-cc auth --json --poll

# poll, still waiting (spinner state, not a failure)
CT_AUTH_SCENARIO=pending scripts/tests/fixtures/mock-cc auth --json --poll

# simulate the env-error / exit-2 path
CT_AUTH_SCENARIO=exit-2 scripts/tests/fixtures/mock-cc auth --json --poll

# the ONE adversarial negative-test scenario — see below
CT_AUTH_SCENARIO=authorized-leaked-field-adversarial scripts/tests/fixtures/mock-cc auth --json --poll
```
...
```bash
scripts/tests/fixtures/wizard/validate.sh
```
...
```bash
CT_CLI_PATH=/absolute/path/to/scripts/tests/fixtures/mock-cc \
...
```bash
scripts/tests/fixtures/validate.sh          # sibling doctor corpus — unchanged, still green
scripts/tests/fixtures/wizard/validate.sh   # this corpus — auth seam shape + scenario corpus
```
