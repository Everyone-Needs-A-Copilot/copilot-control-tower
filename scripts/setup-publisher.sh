...
VENDORED_CC_PATH="packaging/cc/cc"
APPLE_ID=""
SIGN_IDENTITY=""
TEAM_ID=""
SKIP_NOTARY=0
...
Options:
  ...
  --vendored-cc PATH    Verified signed cc helper (default: packaging/cc/cc).
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --env-file)
            ...
            ;;
        --vendored-cc)
            [[ $# -ge 2 ]] || die "--vendored-cc requires a value"
            VENDORED_CC_PATH="$2"
            shift 2
            ;;
        --skip-notary)
...
case "${VENDORED_CC_PATH}" in
    /*) ;;
    *) VENDORED_CC_PATH="${REPO_ROOT}/${VENDORED_CC_PATH}" ;;
esac
[[ -f "${VENDORED_CC_PATH}" && ! -L "${VENDORED_CC_PATH}" ]] ||
    die "--vendored-cc must name a regular, non-symlink file"
scripts/verify-vendored-cc.sh --release "${VENDORED_CC_PATH}"
...
umask 077
{
    ...
    printf "export CT_VENDORED_CC_PATH=%s\n" "$(quote_sh "${VENDORED_CC_PATH}")"
} > "${tmp_file}"
...
echo
echo "Next:"
echo "  ./scripts/package-user-release --source-ref main"
