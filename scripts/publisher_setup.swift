...
final class PublisherSetupModel: ObservableObject {
    ...
    @Published var vendoredCCPath: String = "packaging/cc/cc"
    ...
    func setup() {
        ...
        let trimmedVendoredCC = vendoredCCPath.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        guard !trimmedVendoredCC.isEmpty else {
            status = "Choose the verified cc helper."
            return
        }
        if !skipNotary {
        ...
        let vendoredCC = trimmedVendoredCC
    ...
    nonisolated private static func writeEnvFile(
        path: String,
        identity: String,
        profileName: String,
        vendoredCCPath: String,
        replaceExisting: Bool
    ) throws {
        let url = URL(fileURLWithPath: path, relativeTo: URL(fileURLWithPath: FileManager.default.currentDirectoryPath)).standardizedFileURL
        let ccURL = URL(
            fileURLWithPath: vendoredCCPath,
            relativeTo: URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        ).standardizedFileURL
        var ccMetadata = stat()
        guard lstat(ccURL.path, &ccMetadata) == 0,
              (ccMetadata.st_mode & S_IFMT) == S_IFREG,
              (ccMetadata.st_mode & 0o022) == 0 else {
            throw NSError(
                domain: "PublisherSetup",
                code: 3,
                userInfo: [NSLocalizedDescriptionKey: "The cc helper must be a regular, non-symlink file that is not group/world writable."]
            )
        }
        ...
        export CT_VENDORED_CC_PATH=\(shellQuote(ccURL.path))
    ...
    private var formView: some View {
        stepShell(
            ...
            sectionCard("Release config") {
                ...
                formFieldRow("Verified cc helper") {
                    TextField("packaging/cc/cc", text: $model.vendoredCCPath)
                        .textFieldStyle(.roundedBorder)
                } caption: {
                    "A separately obtained signed helper matching packaging/cc/VERSION and PINNED_SHA256. The helper is not committed to this source repository."
                }
