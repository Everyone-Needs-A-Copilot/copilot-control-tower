...
DEFAULT_DOSSIER_ROOT = (
    Path(
        os.environ.get("CC_KNOWLEDGE_REPO")
        or next(
            (
                str(c / "knowledge-copilot-internal")
                for c in (Path("/Volumes/Dev/Sites/CSE"), Path("/Users/pabs/Sites/CSE"))
                if c.is_dir()
            ),
            "/Volumes/Dev/Sites/CSE/knowledge-copilot-internal",
        )
    )
    / "02-products"
    / "04-applications"
