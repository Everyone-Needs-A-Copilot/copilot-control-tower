...
1. **Parse, never compute.** Control Tower calls CLI verbs (`copilot auth/doctor/update/resolve/deprovision/freshness/layers`, `cc onboard/workspace`) via a **versioned `--json` contract** and renders the result. It contains **no** resolution, sync, signature, merge, or wipe logic of its own. If a decision requires computing ecosystem state, it belongs in the CLI, not here. There is no standalone `repair` or `publish` verb: Git history remediation lives inside `cc onboard`'s own routing, and a first-class `repair` verb plus the author-side `publish` push path are both formally deferred — see [`ADR-008`](docs/40-initiatives/02-enac-self-onboarding/decisions/ADR-008-repair-and-publish-deferred.md). (If `publish` is ever built, the same rule holds: the merge-conflict chooser would be CLI-computed — the app only renders the options and passes the choice back.) See [`docs/01-architecture/cli-contract.md`](docs/01-architecture/cli-contract.md).
...
This machine is fully provisioned. Standing credentials that exist and must be assumed present: the `ct-notary` `notarytool` keychain profile (team `3SYGVX2HB8`), the Developer ID Application signing identity, `~/.ssh/enac_foundation_release` (foundation signing), keychain service `copilot-cli` entries (Infisical machine identity + service tokens), and `gh auth` with `admin:org`. `notarytool` uses the macOS **Data Protection Keychain** unless an explicit file-based keychain is supplied, so `security find-generic-password` against `login.keychain-db` is not a valid existence test. The authoritative probe is `xcrun notarytool history --keychain-profile ct-notary --output-format json`; retry it and re-run it from the logged-in user's fresh Terminal session before treating the profile as unavailable. A **local** "No Keychain password item found" means the item was unavailable to that process, not that it was deleted. If any later `notarytool` probe succeeds, continue the release without asking the owner to recreate credentials or rerun Publisher Setup. A **remote** rejection of a credential that reads fine locally, evidenced by server-side logs (for example HTTP 401), is a different condition and may require owner action. Never work around a credential check to get past either case: no `--skip-verify`, no `--force`, consistent with invariant #4. See the publisher runbook's credential-troubleshooting section.
